...
static void signal_handler(int sig)
{
    (void)sig;
    g_running = false;
    AIRY_LOG_INFO("Plugin_d: received shutdown signal");
}
...
int main(int argc, char *argv[])
{
    ...
    AIRY_LOG_INFO("============================================");
    AIRY_LOG_INFO("Plugin_d: starting (P2.2 Plugin Management)");
    AIRY_LOG_INFO("============================================");
    ...
    if (server_fd < 0) {
        AIRY_LOG_ERROR("Plugin_d: failed to create socket at %s", PLUGIN_D_SOCKET_PATH);
        return EXIT_FAILURE;
    }
    AIRY_LOG_INFO("Plugin_d: listening on %s (fd=%d)", PLUGIN_D_SOCKET_PATH, (int)server_fd);

    /* P1.7: ServiceDiscovery 自动注册 */
    g_bsd = daemon_bootstrap_sd_start("plugin_d", "plugin", PLUGIN_D_SOCKET_PATH,
                                      0, "plugin,core", 0);
    if (!g_bsd) {
        AIRY_LOG_ERROR("Plugin_d: ServiceDiscovery bootstrap failed");
        return EXIT_FAILURE;
    }
    AIRY_LOG_INFO("Plugin_d: ServiceDiscovery registered");

    /* P1.8: IPC Bus 统一消息路由 */
    g_bipc = daemon_bootstrap_ipc_start("plugin_d", "plugin", PLUGIN_D_SOCKET_PATH,
                                        0, IPC_BUS_PROTO_JSON_RPC);
    if (!g_bipc) {
        AIRY_LOG_ERROR("Plugin_d: IPC Bus bootstrap failed");
        daemon_bootstrap_sd_stop(g_bsd);
        return EXIT_FAILURE;
    }
    AIRY_LOG_INFO("Plugin_d: IPC Bus registered");
    ...
    if (plugin_permission_init(&perm_cfg) != 0) {
        AIRY_LOG_WARN("Plugin_d: Permission module init failed");
    }
    ...
    if (plugin_discovery_init(&disc_cfg) != 0) {
        AIRY_LOG_ERROR("Plugin_d: Plugin discovery init failed");
    }
    ...
    if (plugin_discovery_scan(&results, &count) == 0 && count > 0) {
        AIRY_LOG_INFO("Plugin_d: Found %zu plugins:", count);
        for (size_t i = 0; i < count; i++) {
            if (results[i].valid) {
                AIRY_LOG_INFO("  [%zu] %s v%s (type=%d, perms=%u)",
                                 i + 1, results[i].name,
        ...
        for (size_t i = 0; i < count; i++) {
            ...
            if (perm_result != PLUGIN_PERM_ALLOWED) {
                AIRY_LOG_WARN("Plugin_d: Skipping '%s' — permission denied: %s",
            ...
            if (load_ret == 0) {
                ...
                loaded++;
                AIRY_LOG_INFO("Plugin_d: Loaded and started '%s'",
                                 results[i].name);
            } else {
                AIRY_LOG_ERROR("Plugin_d: Failed to load '%s' from '%s'",
                                  results[i].name, results[i].library_path);
            }
        }

        AIRY_LOG_INFO("Plugin_d: Loaded %zu/%zu plugins", loaded, count);
        plugin_discovery_free_results(results, count);
    } else {
        AIRY_LOG_INFO("Plugin_d: No plugins found in '%s'",
                         disc_cfg.plugins_dir);
    }

    AIRY_LOG_INFO("Plugin_d: Entering event loop...");
