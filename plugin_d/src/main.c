...
DAEMON_DECLARE_COMMON(plugin_d, plugin, DEFAULT_SOCKET_PATH_UNIX, DEFAULT_SOCKET_PATH_WIN,
...
static void handle_load(cJSON *params, int id, airy_sock_t client_fd)
{
    ...
    int ret =
        plugin_service_load(library_path->valuestring,
                            config_path && cJSON_IsString(config_path) ? config_path->valuestring :
                                                                         NULL,
                            &out_name);
    if (ret != 0 || !out_name) {
...
static void handle_start(cJSON *params, int id, airy_sock_t client_fd)
{
    ...
    if (ret != 0) {
        ...
        SVC_LOG_ERROR("plugin.start failed: name=%s error=%d", name->valuestring, ret);
        return;
    }
...
static void handle_stop(cJSON *params, int id, airy_sock_t client_fd)
{
    ...
    if (ret != 0) {
        ...
        SVC_LOG_ERROR("plugin.stop failed: name=%s error=%d", name->valuestring, ret);
        return;
    }
...
static void handle_execute(cJSON *params, int id, airy_sock_t client_fd)
{
    ...
    if (ret != 0) {
        ...
        SVC_LOG_ERROR("plugin.execute failed: name=%s error=%d", name->valuestring, ret);
        return;
    }
    ...
    JSONRPC_SEND_SUCCESS(client_fd, result, id);
    SVC_LOG_INFO("plugin.execute OK: name=%s output_len=%zu", name->valuestring,
    ...
    for (size_t i = 0; i < g_discovered_count; i++) {
        ...
        plugin_permission_result_t perm_result =
            plugin_permission_check((const char (*)[64])g_discovered[i].permissions,
                                    g_discovered[i].permission_count, g_discovered[i].name, denied,
                                    sizeof(denied));

        if (perm_result != PLUGIN_PERM_ALLOWED) {
            SVC_LOG_WARN("Plugin_d: skipping '%s' — permission denied: %s", g_discovered[i].name,
                         denied);
            continue;
        }

        const char *out_name = NULL;
        int load_ret = plugin_service_load(g_discovered[i].library_path, NULL, &out_name);
        if (load_ret == 0) {
            plugin_service_start(g_discovered[i].name);
            loaded++;
            SVC_LOG_INFO("Plugin_d: loaded and started '%s'", g_discovered[i].name);
        } else {
            SVC_LOG_ERROR("Plugin_d: failed to load '%s' from '%s' (error=%d)",
                          g_discovered[i].name, g_discovered[i].library_path, load_ret);
        }
    }
    SVC_LOG_INFO("Plugin_d: loaded %zu/%zu plugin(s)", loaded, g_discovered_count);
}
...
int main(int argc, char **argv)
{
    ...
    int parse_rc = daemon_parse_args(argc, argv, &config_path, &use_tcp, print_usage_plugin_d);
    if (parse_rc > 0)
    ...
    SVC_LOG_INFO("Plugin_d: listening on %s (fd=%d)", use_tcp ? "TCP" : DEFAULT_SOCKET_PATH_UNIX,
                 (int)server_fd);
    ...
    int ret =
        daemon_init_event_driver("plugin_d", "plugin", sock_addr, use_tcp ? DEFAULT_TCP_PORT : 0,
                                 "plugin,core", use_tcp, &ev_config, &g_event_driver_plugin_d,
    ...
    method_dispatcher_register(g_dispatcher_plugin_d, "shutdown", on_shutdown_method_plugin_d,
                               NULL);
    SVC_LOG_INFO("Plugin_d: registered 13 RPC methods (plugin.* namespace)");

    if (daemon_event_driver_add_server_fd(g_event_driver_plugin_d, (int)server_fd) != 0) {
