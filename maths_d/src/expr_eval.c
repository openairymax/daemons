...
/* MSVC /O2(/Oi) 会把 sqrt/sin 等 libm 符号 intrinsic 化，取地址不再是
 * 常量表达式 → 静态函数指针表 C2099（#122 实证 expr_eval.c(145,29)）。
 * pragma function 让其回落真实函数地址；须在文件作用域（函数内会触发
 * C2156），且仅 MSVC 需要（clang 对未知 pragma 告警，故守卫）。 */
#ifdef _MSC_VER
