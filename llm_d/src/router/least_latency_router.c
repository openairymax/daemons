...
int route_least_latency(const llm_route_request_t *request, llm_route_result_t *result)
{
    ...
    AIRY_LOG_DEBUG("C-L02: Latency: evaluating %zu endpoints for latency", eligible_count);

    size_t best_idx = 0;
    uint32_t best_latency = UINT32_MAX;

    for (size_t i = 0; i < eligible_count; i++) {
        AIRY_LOG_DEBUG("C-L02: Latency: endpoint[%zu] %s/%s latency=%ums", i,
    ...
    router_fill_result(result, ep, LLM_ROUTE_LATENCY, 85, input_tokens, output_tokens);
    ...
    if (eligible_count > 1) {
        ...
        if (second_best != UINT32_MAX) {
            ...
            AIRY_LOG_DEBUG("C-L02: Latency: fallback set to %s/%s "
                           ...
                           eligible[fallback_idx]->model_name, second_best);
        }
    }

    AIRY_LOG_INFO("C-L02: Latency: selected %s/%s latency=%ums", ep->provider_name, ep->model_name,
                  best_latency);
