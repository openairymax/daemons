...
static complexity_level_t assess_complexity(const char *input)
{
    ...
    const char *complex_kw[] = {"architecture", "distributed", "system design",
                                "scalability",  "架构",        "分布式",
                                "系统设计",     "高可用",      "微服务"};
    const char *moderate_kw[] = {"function", "algorithm", "sort", "implement", "write",  "函数",
                                 "算法",     "排序",      "实现", "编写",      "Python", "Java"};
    ...
    for (size_t i = 0; i < sizeof(moderate_kw) / sizeof(moderate_kw[0]); i++) {
        if (strstr(input, moderate_kw[i]))
            moderate_score++;
    }

    if (complex_score >= 1 || len > 500)
        return COMPLEXITY_COMPLEX;
    if (moderate_score >= 1 || len > 50)
...
static void test_routing_logging(void)
{
    ...
    for (int i = 0; i < 3; i++) {
        ...
        snprintf(entry->reason, sizeof(entry->reason), "Complexity=%s, len=%zu, routed to %s",
    ...
    for (int i = 0; i < log_count; i++) {
        printf("    Log[%d]: summary=\"%s\" complexity=%s model=%s\n", i,
               log_entries[i].input_summary, complexity_names[log_entries[i].complexity],
