...
static ipc_bp_level_t compute_level(size_t depth, size_t capacity, const ipc_bp_config_t *cfg)
{
    if (capacity == 0)
...
ipc_bp_controller_t *ipc_bp_create(const ipc_bp_config_t *config)
{
    ...
    SVC_LOG_INFO("P1.24: IPC backpressure created (capacity=%zu, slow=%u%%, drop=%u%%, "
                 "reject=%u%%, recover=%u%%)",
                 ctrl->config.queue_capacity, ctrl->config.slow_threshold_pct,
                 ctrl->config.drop_threshold_pct, ctrl->config.reject_threshold_pct,
...
void ipc_bp_destroy(ipc_bp_controller_t *ctrl)
{
    ...
    SVC_LOG_INFO("P1.24: IPC backpressure destroyed (sent=%zu, dropped=%zu, rejected=%zu)",
                 (size_t)ctrl->stats.total_sent, (size_t)ctrl->stats.total_dropped,
...
ipc_bp_level_t ipc_bp_update(ipc_bp_controller_t *ctrl, size_t current_depth)
{
    ...
    ipc_bp_level_t new_level =
        compute_level(current_depth, ctrl->config.queue_capacity, &ctrl->config);
    ...
    if (new_level != old_level) {
        ...
        if (new_level > old_level) {
            ...
            SVC_LOG_WARN("P1.24: IPC backpressure %s → %s (depth=%zu/%zu, %.1f%%)",
                         level_names[old_level], level_names[new_level], current_depth,
                         ctrl->config.queue_capacity,
                         ctrl->config.queue_capacity > 0 ?
                             (double)current_depth * 100.0 / ctrl->config.queue_capacity :
                             0.0);

            if (new_level == IPC_BP_REJECT) {
                SVC_LOG_ERROR("C-L02: BP: UPDATE level=REJECT depth=%zu/%zu sent=%zu dropped=%zu "
                              "rejected=%zu",
...
bool ipc_bp_should_send(ipc_bp_controller_t *ctrl, bool is_droppable)
{
    ...
    case IPC_BP_REJECT:
        ...
        if (is_droppable) {
            ctrl->stats.total_dropped++;
            SVC_LOG_WARN(
                "C-L02: BP: DROP level=REJECT depth=%zu/%zu sent=%zu dropped=%zu rejected=%zu",
                (size_t)ctrl->stats.queue_depth, ctrl->stats.queue_capacity,
                (size_t)ctrl->stats.total_sent, (size_t)ctrl->stats.total_dropped,
                (size_t)ctrl->stats.total_rejected);
            return false;
...
bool ipc_bp_should_accept_connection(ipc_bp_controller_t *ctrl)
{
    ...
    if (ctrl->stats.current_level >= IPC_BP_REJECT) {
        ...
        SVC_LOG_WARN("C-L02: BP: REJECT-ACCEPT level=%d depth=%zu/%zu rejected=%zu",
                     (int)ctrl->stats.current_level, (size_t)ctrl->stats.queue_depth,
                     ctrl->stats.queue_capacity, (size_t)ctrl->stats.total_rejected);
        return false;
    }
