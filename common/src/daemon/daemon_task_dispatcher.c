...
static void dispatch_worker(void *arg)
{
    ...
    if (session->cancel_flag) {
        ...
        ctx->result_slot->error = AIRY_STRDUP("cancelled");
                                                  ...
                                                  const parallel_dispatch_config_t *config)
{
    ...
    parallel_dispatcher_t *disp =
        (parallel_dispatcher_t *)AIRY_CALLOC(1, sizeof(parallel_dispatcher_t));
    if (!disp) {
                                          ...
                                          parallel_complete_cb_t on_complete, void *user_data)
{
    ...
    airy_mtx_init(&session->lock);
    airy_cond_init(&session->cond);
    ...
    *results = (parallel_result_t *)AIRY_CALLOC(task_count, sizeof(parallel_result_t));
    ...
    for (size_t i = 0; i < task_count; i++) {
        ...
        while (thread_pool_active_count(dispatcher->pool) >= dispatcher->config.max_concurrency) {
            ...
            airy_cond_timedwait(&session->cond, &session->lock, 100);
        }
        airy_mtx_unlock(&session->lock);
        ...
        if (rc != 0) {
            ...
            session->contexts[i].result_slot->error = AIRY_STRDUP("submit failed");
...
wait_done: {
    ...
    while (1) {
        ...
        airy_mtx_lock(&session->lock);
        airy_cond_timedwait(&session->cond, &session->lock, 50);
        airy_mtx_unlock(&session->lock);
    }
}
...
void parallel_result_free(parallel_result_t *results, size_t count)
{
    ...
    for (size_t i = 0; i < count; i++) {
        if (results[i].output)
            AIRY_FREE(results[i].output);
        if (results[i].error)
            AIRY_FREE(results[i].error);
    }
    AIRY_FREE(results);
}
...
parallel_task_t parallel_task_create(const char *tool_id, const char *params_json)
{
    ...
    task.tool_id = tool_id ? AIRY_STRDUP(tool_id) : NULL;
    task.params_json = params_json ? AIRY_STRDUP(params_json) : NULL;
    task.user_data = NULL;
    return task;
}
...
void parallel_task_free(parallel_task_t *task)
{
    if (!task)
        return;
    if (task->tool_id) {
        AIRY_FREE(task->tool_id);
        task->tool_id = NULL;
    }
    if (task->params_json) {
        AIRY_FREE(task->params_json);
