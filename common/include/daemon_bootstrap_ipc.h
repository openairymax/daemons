/* SPDX-FileCopyrightText: 2025-2026 SPHARX Ltd. */
/* SPDX-License-Identifier: AGPL-3.0-or-later OR Apache-2.0 */

/**
 * @file daemon_bootstrap_ipc.h
 * @brief P1.8 C-L09: daemon IPC Bus one-call bootstrap module - re-export
 *        compat header.
 *
 * P0.17 phase 4: the real definitions moved to
 * commons/utils/ipc/daemon_bootstrap_ipc.h. This file is only a
 * backward-compatible re-export layer.
 */

#ifndef AIRY_RT_DAEMON_COMMON_DAEMON_BOOTSTRAP_IPC_H
#define AIRY_RT_DAEMON_COMMON_DAEMON_BOOTSTRAP_IPC_H

#include "../../../commons/utils/ipc/daemon_bootstrap_ipc.h"

#endif /* AIRY_RT_DAEMON_COMMON_DAEMON_BOOTSTRAP_IPC_H */
