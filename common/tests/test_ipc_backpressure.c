...
static void test_bp_full_cycle_simulation(void)
{
    ...
    TEST_ASSERT(ipc_bp_should_accept_connection(ctrl) == true,
                "connections accepted after recovery");
    ...
    printf("    Stats: sent=%zu, dropped=%zu, rejected=%zu, slow_down=%zu, recover=%zu\n",
           (size_t)stats.total_sent, (size_t)stats.total_dropped, (size_t)stats.total_rejected,
           (size_t)stats.slow_down_events, (size_t)stats.recover_events);
...
static void test_bp_null_safety(void)
{
    ...
    TEST_ASSERT(ipc_bp_should_send(NULL, true) == true,
                "should_send(NULL, droppable) should return true");
    TEST_ASSERT(ipc_bp_should_accept_connection(NULL) == true,
                "should_accept(NULL) should return true");
