...
int main(void)
{
    ...
    if (test_safe_strcpy() != 0)
        failed++;
    if (test_safe_strcat() != 0)
        failed++;
    if (test_safe_sprintf() != 0)
        failed++;
    if (test_safe_strlen() != 0)
        failed++;
    if (test_safe_strcmp() != 0)
        failed++;
    if (test_safe_strdup_with_limit() != 0)
        failed++;
    /* if (test_is_valid_ascii() != 0) failed++; */
    if (test_secure_clear() != 0)
        failed++;
