...
static void test_terminate(void)
{
    ...
    ret = agent_service_invoke(svc, agent_id, "ping", 4, NULL, 0, NULL, &out_output);
    assert(ret != AIRY_SUCCESS);
...
static void test_invoke(void)
{
     ...
     * success */
    ret = agent_service_invoke(svc, agent_id, "hello", 5, NULL, 0, NULL, &out_output);
    assert(ret != AIRY_SUCCESS);
    assert(out_output != NULL);
    assert(strstr(out_output, "error") != NULL);
...
static void test_invoke_nonexistent(void)
{
    ...
    char *out_output = NULL;
    int ret = agent_service_invoke(svc, "nonexistent_agent_id", "hi", 2, NULL, 0, NULL,
                                   &out_output);
...
static void test_spawn_after_terminate(void)
{
    ...
    char *out_output = NULL;
    int irc = agent_service_invoke(svc, r2, "hi", 2, NULL, 0, NULL, &out_output);
...
static void test_invoke_cancel(void)
{
    ...
    char *out_output = NULL;
    ret = agent_service_invoke(svc, agent_id, "ping", 4, NULL, 0, &token, &out_output);
